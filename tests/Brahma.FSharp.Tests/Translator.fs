...
let basicLocalIdTests =
    testList
        "Basic tests on LocalID translation"
        [
            testCase "LocalID of 1D"
            <| fun _ ->
                let command =
...
let basicBinOpsTests =
    testList
        "Basic operations translation tests"
        [
            ...
            testCase "Binary operations. Math."
            <| fun _ ->
                let command =
                    <@ fun (range: _1D) (buf: array<int>) ->
                        let x = 0
...
let controlFlowTests =
    testList
        ...
        [
            ...
            ptestCase
                "WHILE with single statement in the body and this stetement is assignment of constant. This test translates to openCL correctly but breaks openCL compiler on ubuntu 18.04"
            <| fun _ ->
                let command =
                    <@ fun (range: _1D) (buf: array<int>) ->
                        while true do
            ...
            testCase "WHILE with complex condition"
            <| fun _ ->
                let command =
                    <@ fun (range: _1D) (buf: array<int>) ->
...
let namesResolvingTests =
    testList
        "Tests on variables renaming."
        [
            testCase "Bindings with equal names."
            <| fun _ ->
                let command =
                    <@ fun (range: _1D) (buf: array<int>) ->
                        let x = 2
            ...
            testCase "Binding and FOR counter conflict 1."
            <| fun _ ->
                let command =
                    <@ fun (range: _1D) (buf: array<int>) ->
                        let i = 2
            ...
            testCase "Binding and FOR counter conflict 2."
            <| fun _ ->
                let command =
                    <@ fun (range: _1D) (buf: array<int>) ->
                        for i in 1 .. 2 do
            ...
            testCase "Binding and FOR counter conflict 3."
            <| fun _ ->
                let command =
                    <@ fun (range: _1D) (buf: array<int>) ->
                        for i in 0 .. 1 do
            ...
            testCase "Binding and FOR counter conflict 4."
            <| fun _ ->
                let command =
                    <@ fun (range: _1D) (buf: array<int>) ->
                        let i = 1
...
let constantArrayTests =
    testList
        "Constant array translation tests."
        [
            testCase "Constant array translation. Test 1"
            <| fun _ ->
                let cArray1 = [| 1; 2; 3 |]
            ...
            testCase "Constant array translation. Test 2"
            <| fun _ ->
                let cArray1 = [| 1; 2; 3 |]
...
let lambdaLiftingTests =
    testList
        ...
        [
            ...
            ptestCase "Template Let Transformation Test 14"
            <| fun _ ->
                let command =
                    <@ fun (range: _1D) (buf: array<int>) ->
                        let f (y: int) =
...
let curryingTests =
    testList
        "Currying translation test"
        [
            ptestCase "Nested functions. Carring 1."
            <| fun _ ->
                let command =
                    <@ fun (range: _1D) (buf: array<int>) ->
                        let f x y = x - y
            ...
            testCase "Nested functions. Currying 2."
            <| fun _ ->
                let command =
                    <@ fun (range: _1D) (buf: array<int>) ->
                        let f x y =
...
let localMemoryTests =
    testList
        "Test of local memory declaration functions."
        [
            testCase "Local int"
            <| fun _ ->
                let command =
...
let localMemoryAllocationTests =
    ptestList
        "Translation of local memory allocation functions."
        [
            ptestCase "Constant array translation. Local copy test 1"
            <| fun _ ->
                let cArray1 = [| 1; 2; 3 |]
