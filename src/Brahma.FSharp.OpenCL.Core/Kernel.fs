...
type GpuKernel<'TRange, 'a, 't when 'TRange :> INDRangeDimension>
    ...
    let compileQuery translatorOptions additionalSources =
        ...
        let error =
            Cl.BuildProgram(program, 1u, [| device |], options, null, System.IntPtr.Zero)
    ...
    let createKernel program =
        let (clKernel, error) =
            Cl.CreateKernel(program, kernelName)
    ...
    let toIMem a =
        ...
        | :? int as i -> System.IntPtr(System.Runtime.InteropServices.Marshal.SizeOf i), box i
        ...
        | DerivedPatterns.Lambdas (lambdaArgs, _) ->
            ...
            let argsWithoutMutexes =
                ...
                |> List.map (fun v ->
                    ...
                    then
                        let vName = v.Name
                        let vType = typedefof<ClBuffer<_>>.MakeGenericType(v.Type.GetElementType())
