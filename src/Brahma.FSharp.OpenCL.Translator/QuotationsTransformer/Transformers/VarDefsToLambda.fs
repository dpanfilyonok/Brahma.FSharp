module Brahma.FSharp.OpenCL.QuotationsTransformer.Transformers.LetVarAbstracter
...
let rec varDefsToLambda (expr: Expr) =
    match expr with
    | LetVar (var, letExpr, inExpr) ->
        if isPrimitiveExpression letExpr
        then
            ...
            let letExprNew =
                Expr.Let(
                    fVar,
                    Expr.Lambda(
                        Var("unitVar", typeof<unit>),
                        varDefsToLambda letExpr
                    ),
    ...
    | ExprShape.ShapeLambda (var, body) ->
        Expr.Lambda(var, varDefsToLambda body)
