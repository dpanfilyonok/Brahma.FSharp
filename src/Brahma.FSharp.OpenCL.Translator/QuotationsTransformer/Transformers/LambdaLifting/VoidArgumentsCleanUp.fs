module Brahma.FSharp.OpenCL.QuotationsTransformer.Transformers.LambdaLifting.VoidArgumentsCleanUp
...
let rec private cleanUpVoidArgumentsImpl (subst: Map<Var, Var>) (expr: Expr) =
    ...
    | Patterns.LetFuncUncurry (var, args, body, inExpr) ->
        let args' =
            ...
            then args
            else List.filter (not << Common.isTypeOf<unit>) args

        let newFuncVarType = Common.makeFunctionType body.Type <| List.map (fun (var: Var) -> var.Type) args'
        ...
        Expr.Let(newFuncVar, Common.makeLambdaExpr args' body', inExpr')
    ...
    | Patterns.ApplicationUncurry (head, exprs) ->
        ...
        | Patterns.Var var ->
            ...
            | Some var' ->
                let exprs' =
                    if isConsistOfVoidExprOnly exprs
                    then exprs
                    else List.filter (fun (exp: Expr) -> exp.Type <> typeof<unit>) exprs
                Common.makeApplicationExpr
                    <| Expr.Var var'
