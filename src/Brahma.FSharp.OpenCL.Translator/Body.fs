...
module Body =
    let private clearContext (targetContext: TargetContext<'a,'b>) =
        ...
        for td in targetContext.TupleDecls do context.TupleDecls.Add(td.Key, td.Value)
        for t in targetContext.TupleList do context.TupleList.Add(t)
        for kvp in targetContext.UserDefinedStructsOpenCLDeclaration do context.UserDefinedStructsOpenCLDeclaration.Add (kvp.Key,kvp.Value)
        for kvp in targetContext.UserDefinedUnionsOpenCLDeclaration do context.UserDefinedUnionsOpenCLDeclaration.Add (kvp.Key,kvp.Value)
